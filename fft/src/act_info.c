...
sprintf(buf7,"{gL{R%2d{c %3d{bexp{W             | PA MA BR FA MV JM SP |",ch->level,((ch->level + 1) * 100) - ch->exp);
 
printf_to_char(ch,"\n\r|-----------------------------------------------|\n\r");
printf_to_char(ch,"| %s%-12s{W           |{r%4d{c/{r%4d{RHP {g%3d{c/{g%3d{GMP{W |\n\r",get_t_color(ch),fcn(ch),ch->hit,ch->max_hit,ch->mana,ch->max_mana);
printf_to_char(ch,"| %-66s\n\r",buf7);
printf_to_char(ch,"|                        | %2d %2d %2d %2d %2d %2d %2d {W|\n\r",ch->PhysAttack,ch->MagAttack,
ch->Brave,ch->Faith,ch->MoveLeft,ch->jump,ch->speed);
...
return;
    ...
    sprintf (buf,
             ...
             ch->PhysAttack,
             ch->Faith,
             get_curr_stat (ch, STAT_FAITH),
             ch->MagAttack,
             ch->MagAttack,
             ch->perm_stat[STAT_DEX],
             get_curr_stat (ch, STAT_DEX),
             ch->Brave, get_curr_stat (ch, STAT_BRAVE));
    send_to_char (buf, ch);
...
void do_who (CHAR_DATA * ch, char *argument)
{
    ...
    for (d = descriptor_list; d != NULL; d = d->next)
    {
        ...
        /*for fights*/
sprintf(buf, "{y[{G%2d %1s %1s {b%-8s{y ]{x[{x%s{x{y{x{x]{W%s%s\n",wch->level,
wch->sex == SEX_MALE ? "{BM{x" : "{MF{x",wch->infight ? "{R*{x" : "{c-{x",job_table[wch->jobid].name,wch->fdisp,wch->name,wch->pcdata->title);
add_buf(output,buf);
    }
...
void do_equipment (CHAR_DATA * ch, char *argument)
{
    ...
    send_to_char("Left  : ",ch);
    if ((obj = get_eq_char (ch, WEAR_LEFT)) != NULL)
    send_to_char (format_obj_to_char (obj, ch, TRUE), ch); 
    else
    send_to_char("None",ch);
    send_to_char("\n\r",ch);

    send_to_char("Body  : ",ch);
    ...
    send_to_char("Acc   : ",ch);
    if ((obj = get_eq_char (ch, WEAR_ACCESSORY)) != NULL)
    send_to_char (format_obj_to_char (obj, ch, TRUE), ch); 
    else
    send_to_char("None",ch);
    send_to_char("\n\r",ch);
