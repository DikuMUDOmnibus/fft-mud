...
void do_mstat (CHAR_DATA * ch, char *argument)
{
    ...
    sprintf (buf,
             ...
             victim->perm_stat[STAT_PA],
             get_curr_stat (victim, STAT_PA),
             victim->perm_stat[STAT_MA],
             get_curr_stat (victim, STAT_MA),
             victim->perm_stat[STAT_FAITH],
             get_curr_stat (victim, STAT_FAITH),
             victim->perm_stat[STAT_DEX],
             get_curr_stat (victim, STAT_DEX),
             victim->perm_stat[STAT_BRAVE], get_curr_stat (victim, STAT_BRAVE));
    send_to_char (buf, ch);
...
switch(ch->sex)
{
case SEX_MALE:    ch->SpP = 98304; ch->PAP = 81920; ch->MAP = 65536;
...
case SEX_FEMALE : ch->SpP = 98304; ch->PAP = 65536; ch->MAP = 81920;
...
void do_mset (CHAR_DATA * ch, char *argument)
{
    ...
    if (arg1[0] == '\0' || arg2[0] == '\0' || arg3[0] == '\0')
    {
        ...
        send_to_char ("    jp align train thirst hunger drunk full\n\r", ch);
    ...
    if (!str_cmp (arg2, "str"))
    {
        if (value < 1 || value > 99)
        {
            sprintf (buf,"Range is 1 to 99.\n\r.");
            send_to_char (buf, ch);
            return;
        }

        victim->perm_stat[STAT_PA] = value;
        return;
    }
	...
	DIE(ch,"Not on NPC's.\n\r");
        for(i=0;i!=MAX_JOB;i++)
        victim->job_jp[i] = value;
        return;
    }

    if (!str_cmp (arg2, "joblevel"))
    {                            /* OLC */
    int i;
        if (IS_NPC (victim))
	DIE(ch,"Not on NPC's.\n\r");
    ...
    if (!str_cmp (arg2, "int"))
    {
        ...
        victim->perm_stat[STAT_MA] = value;
        return;
    }
    ...
    if (!str_cmp (arg2, "wis"))
    {
        ...
        victim->perm_stat[STAT_FAITH] = value;
        return;
    }
    ...
    if (!str_cmp (arg2, "con"))
    {
        ...
        victim->perm_stat[STAT_BRAVE] = value;
        return;
    }
    ...
    if (!str_prefix (arg2, "hp"))
    {
        ...
        {
            send_to_char ("Hp range is -10 to 9999 hit points.\n\r", ch);
            return;
        }
    ...
    if (!str_prefix (arg2, "mana"))
    {
        ...
        {
            send_to_char ("Mana range is 0 to 999 mana points.\n\r", ch);
            return;
        }
    ...
    for (;;)
    {
        ...
        {                        /* ok! */
            ...
            if (!d->character->in_room)
                d->character->in_room = get_room_index (100);
...
void qmconfig_read (void) {
	...
	logf("Loading configuration settings from qmconfig.rc...");

	fp = fopen("../area/qmconfig.rc","r");
	if (!fp) {
		logf("qmconfig.rc not found. Using compiled-in defaults.");
		return;
	}
	...
	for(;;) {
		...
		if (!fMatch) {
			logf("qmconfig_read: no match for %s!", word);
			fread_to_eol(fp);
		}
		logf("Settings read");
	}	
