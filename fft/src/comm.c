...
int main (int argc, char **argv)
{
    ...
    boot_db ();
    logf ("ROM is ready to rock on port %d (%s).", port, mud_ipaddress);
...
int init_socket (int port)
{
    ...
    logf("set address to %s", mud_ipaddress);
    ...
    while (*str != '\0')
    {
        ...
        switch (*str)
        {
            ...
            case 'M':
                ...
                break;
            case '3':
                sprintf (buf2, "%3d", ch->AT);
                i = buf2;
                break;
            case '2':
                sprintf (buf2, "%d", ch->mposx);
                i = buf2;
                break;
            case '1':
                sprintf (buf2, "%d", ch->mposy);
                i = buf2;
                break;
            case 'v':
...
void stop_idling (CHAR_DATA * ch)
{
    if (ch == NULL
        ...
        || ch->was_in_room == NULL
        || ch->in_room != get_room_index (2)) return;
...
int colour (char type, CHAR_DATA * ch, char *string)
{
    ...
    switch (type)
    {
        ...
        case '+':
            ...
            strcpy (code, C_B_RED);
            else if(ch->team == TEAM_GREEN)
            strcpy (code, C_B_GREEN);
            else if(ch->team == TEAM_NONE)
