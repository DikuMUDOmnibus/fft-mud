...
sh_int gsn_backstab;
sh_int gsn_counter;
sh_int gsn_counter_tackle;
sh_int gsn_hamedo;
sh_int gsn_move_plus_1;
sh_int gsn_move_plus_2;
sh_int gsn_move_plus_3;
sh_int gsn_dodge;
sh_int gsn_envenom;
sh_int gsn_hide;
sh_int gsn_peek;
...
void load_area (FILE * fp)
{
	...
	logf("Loading area %s", pArea->file_name);
...
#else
		STR_COPY_STR( pMobIndex->affected_by, 
					str_fread_flag( fp, AFF_FLAGS ), AFF_FLAGS );
#endif
...
void load_rooms (FILE * fp)
{
    ...
    for (;;)
    {
        ...
        for (;;)
        {
            ...
            for(gg=0;gg<MAX_MIR;gg++)
            pRoomIndex->monster_vnum[gg] = fread_number (fp);
...
void load_mobprogs (FILE * fp)
{
	...
	logf("Loaded %d mobprogs.", top_mprog_index);
    return;
}
                    ...
                    if (pObjIndex->count < limit || number_range (0, 4) == 0)
                    {
                        ...
                        if (pObj->level > LastMob->level + 3
                            || (pObj->item_type == ITEM_WEAPON
                                ...
                                && pObj->level < 45)) {
                            logf ("Check Levels:");
							logf ("  Object: (VNUM %5d)(Level %2d) %s",
                                     pObj->pIndexData->vnum,
									 pObj->level,
                                     pObj->short_descr);
							logf ("     Mob: (VNUM %5d)(Level %2d) %s",
...
CHAR_DATA *create_mobile (MOB_INDEX_DATA * pMobIndex)
{
    ...
    sprintf(buf,"{G{!%c{X{x",pMobIndex->player_name[0]);     
    mob->fdisp = str_dup (buf);
    mob->name = str_dup (pMobIndex->player_name);    /* OLC */
    mob->Faith = number_range(40,80);
    mob->Brave = number_range(40,80);
    mob->f_name = str_dup(monster_names[number_range(0,255)]);
        ...
        /* load in new style */
    {
        ...
        {
            mob->perm_stat[STAT_PA] += 3;
            mob->perm_stat[STAT_FAITH] -= 1;
            mob->perm_stat[STAT_BRAVE] += 2;
        }
        ...
        if (IS_SET (mob->act, ACT_THIEF))
        {
            ...
            mob->perm_stat[STAT_FAITH] += 1;
            mob->perm_stat[STAT_MA] -= 1;
        }

        if (IS_SET (mob->act, ACT_CLERIC))
        {
            mob->perm_stat[STAT_MA] += 3;
            mob->perm_stat[STAT_DEX] -= 1;
            mob->perm_stat[STAT_PA] += 1;
        }

        if (IS_SET (mob->act, ACT_MAGE))
        {
            mob->perm_stat[STAT_FAITH] += 3;
            mob->perm_stat[STAT_PA] -= 1;
            mob->perm_stat[STAT_DEX] += 1;
        }
...
{
default:
case SEX_MALE:    mob->SpP = 98304; mob->PAP = 81920; mob->MAP = 65536;
jt = number_range(491,524) * 1000;
jt += number_range(287,520);
mob->HPP = jt;   
jt = number_range(229,245) * 1000;
jt += number_range(376,759);
mob->MPP = jt;       
set_stats(mob);
break;

case SEX_FEMALE : mob->SpP = 98304; mob->PAP = 65536; mob->MAP = 81920;
jt = number_range(458,491) * 1000;
jt += number_range(519,752);
mob->HPP = jt;   
jt = number_range(245,262) * 1000;
jt += number_range(143,760);
mob->MPP = jt;
set_stats(mob);
break;
}
        ...
        mob->perm_stat[STAT_PA] += mob->size - SIZE_MEDIUM;
        mob->perm_stat[STAT_BRAVE] += (mob->size - SIZE_MEDIUM) / 2;
