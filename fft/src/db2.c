...
#endif
		STR_OR_STR( pMobIndex->affected_by,
			race_table[pMobIndex->race].aff, AFF_FLAGS );
...
/*    pMobIndex->size            = size_lookup(fread_word(fp)); */
        ...
        for (;;)
        {
            ...
            if (letter == 'F')
            {
        		...
        		{
        			FLAG vector[MAX_FLAGS];
#ifdef BIT_COMPILE
        			STR_COPY_STR( vector, aff_convert_fread_flag( fread_flag( fp ) ), AFF_FLAGS );
				    STR_REMOVE_STR( pMobIndex->affected_by, vector, AFF_FLAGS );
#else
        			STR_COPY_STR( vector, str_fread_flag(fp,AFF_FLAGS), AFF_FLAGS );
				    STR_REMOVE_STR( pMobIndex->affected_by, vector, AFF_FLAGS );
#endif
				}
        ...
        pMobIndex->ai_num = fread_number (fp);
...
void load_objects (FILE * fp)
{
    ...
    for (;;)
    {
        ...
        pObjIndex->tMod = fread_number (fp);
        pObjIndex->tAmount = fread_number (fp);
...
#ifdef BIT_COMPILE
				paf->bitvector			= 
						aff_bit_convert_fread_flag( fread_flag(fp) );
#else
