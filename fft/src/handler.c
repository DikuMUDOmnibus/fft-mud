    ...
    for (loc = 0; loc < MAX_WEAR; loc++)
    {
        ...
        if (!obj->enchanted)
            for (af = obj->pIndexData->affected; af != NULL; af = af->next)
            {
                ...
                {
                    case APPLY_STR:
                        ch->mod_stat[STAT_PA] += mod;
                        break;
                    case APPLY_DEX:
                        ch->mod_stat[STAT_DEX] += mod;
                        break;
                    case APPLY_INT:
                        ch->mod_stat[STAT_FAITH] += mod;
                        break;
                    case APPLY_WIS:
                        ch->mod_stat[STAT_MA] += mod;
                        break;
                    case APPLY_CON:
                        ch->mod_stat[STAT_BRAVE] += mod;
                        break;
...
int can_carry_w (CHAR_DATA * ch)
{
    ...
    return str_app[ch->PhysAttack].carry * 10 + ch->level * 25;
}
...
void affect_modify (CHAR_DATA * ch, AFFECT_DATA * paf, bool fAdd)
{
    ...
    if (fAdd)
    {
        ...
        {
            case TO_AFFECTS:
                STR_SET_BIT (ch->affected_by, paf->bitvector);
                break;
            case TO_IMMUNE:
     ...
     */
    if (!IS_NPC (ch) && (wield = get_eq_char (ch, WEAR_RIGHT)) != NULL
        && get_obj_weight (wield) >
        (str_app[ch->PhysAttack].wield * 10))
    {
        static int depth;
...
void affect_check (CHAR_DATA * ch, int where, int vector)
{
    ...
    for (paf = ch->affected; paf != NULL; paf = paf->next)
        if (paf->where == where && paf->bitvector == vector)
        {
            ...
            {
                case TO_AFFECTS:
                    STR_SET_BIT (ch->affected_by, vector);
                    break;
                case TO_IMMUNE:
...
void char_to_room (CHAR_DATA * ch, ROOM_INDEX_DATA * pRoomIndex)
{
    ...
    if (pRoomIndex == NULL)
    {
        ...
        if ((room = get_room_index (100)) != NULL)
...
char *imm_bit_name (int imm_flags)
{
    ...
    if (imm_flags & IMM_WEAPON)
        strcat (buf, " weapon");
    if (imm_flags & IMM_NON)
        strcat (buf, " non");
    if (imm_flags & IMM_ICE)
        strcat (buf, " ice");
    if (imm_flags & IMM_BOL)
        strcat (buf, " bolt");
    if (imm_flags & IMM_WAT)
        strcat (buf, " water");
    if (imm_flags & IMM_FIR)
        strcat (buf, " fire");
    if (imm_flags & IMM_WIN)
        strcat (buf, " wind");
    if (imm_flags & IMM_DAR)
        strcat (buf, " dark");
    if (imm_flags & IMM_HOL)
        strcat (buf, " holy");
...
char *wear_bit_name (int wear_flags)
{
    ...
    if (wear_flags & ITEM_WEAR_BODY)
        strcat (buf, " body");
    if (wear_flags & ITEM_WEAR_RIGHT)
        strcat (buf, " right");
    if (wear_flags & ITEM_WEAR_LEFT)
        strcat (buf, " left");
    if (wear_flags & ITEM_WEAR_ACCESSORY)
        strcat (buf, " acc");
