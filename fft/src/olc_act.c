...
void show_skill_cmds (CHAR_DATA * ch, int tar)
{
    ...
    for (sn = 0; sn < MAX_SKILL; sn++)
    {
        ...
        if (!str_cmp (skill_table[sn].name, " --- ")
...
REDIT (redit_show)
{
    ...
    strcat (buf1, "Random Battle Slots:\n\r");

    for (door = 0; door < MAX_MIR; door++)
    {
 

    if(pRoom->monster_vnum[door] != 0)
    {

    if ((mob = get_mob_index (pRoom->monster_vnum[door])) == NULL)
    continue;

    cnt++;
        sprintf (buf, "[%2d - %5d %s]\n\r",door, pRoom->monster_vnum[door],mob->short_descr);
        strcat (buf1, buf);
    }

    }
if(cnt==0)
        strcat (buf1, "[ none set ]\n\r");
}
...
argument = one_argument (argument, arg2);
    ...
    pRoom->monster_vnum[atoi(arg)] = atoi(arg2);
        return TRUE;
...
const struct wear_type wear_table[] = {
    {WEAR_NONE, ITEM_TAKE},
    {WEAR_RIGHT, ITEM_WEAR_RIGHT},
    {WEAR_LEFT, ITEM_WEAR_LEFT},
    {WEAR_BODY, ITEM_WEAR_BODY},
    {WEAR_ACCESSORY, ITEM_WEAR_ACCESSORY},
    {NO_FLAG, NO_FLAG}
};
...
/* It somehow fixed a bug in showing scroll/pill/potions too ?! */
        case ITEM_WEAPON:
            ...
            send_to_char (buf, ch);
            sprintf (buf, "[v1] Weapon Power: [%d]\n\r", obj->value[1]);
            send_to_char (buf, ch);
            sprintf (buf, "[v2] Evade:        [%d]\n\r", obj->value[2]);
            send_to_char (buf, ch);
...
OEDIT (oedit_show)
{
    ...
    sprintf (buf, "Modifies:  %s\n\rBy : %d\n\r",
             mod_table[pObj->tMod], pObj->tAmount);
    send_to_char (buf, ch);
...
OEDIT (oedit_modtype)
{
    ...
    if (argument[0] == '\0')
    {
        send_to_char ("Syntax:  modtype (int)\n\r", ch);
        return FALSE;
    }

    pObj->tMod = atoi (argument);

    send_to_char ("Mod set.\n\r", ch);
    return TRUE;
}


OEDIT (oedit_modamount)
{
    OBJ_INDEX_DATA *pObj;
    EDIT_OBJ (ch, pObj);
    ...
    if (argument[0] == '\0')
    {
        send_to_char ("Syntax:  modamount (int)\n\r", ch);
        return FALSE;
    }

    pObj->tAmount = atoi (argument);

    send_to_char ("Amt set.\n\r", ch);
    return TRUE;
}
...
MEDIT (medit_show)
{
  ...
  ,           pMob->vnum,
    ...
    sprintf( buf, "Affected by: [%s]\n\r",
			str_flag_string( affect_flags, pMob->affected_by ) );
    send_to_char (buf, ch);
...
MEDIT (medit_spec)
{
        send_to_char ("Spec functions make baby rabbits explode.\n\r", ch);
        return FALSE;
}
...
{                                /* Moved out of medit() due to naming conflicts -- Hugin */
    ...
    if (argument[0] != '\0')
    {
        ...
        if ((value = str_flag_value (affect_flags, argument)) != NO_FLAG)
        {
            STR_TOGGLE_BIT(pMob->affected_by, value);
...
MEDIT (medit_race)
{
    ...
    if (argument[0] != '\0' && (race = race_lookup (argument)) != 0)
    {
        ...
        STR_OR_STR(pMob->affected_by, race_table[race].aff, AFF_FLAGS);
