...
/* Used with the unlimited bit system */
void STR_DIF( const FLAG *var1, const FLAG *var2, FLAG *buf, int size )
{
	int i;
...
void save_mobile (FILE * fp, MOB_INDEX_DATA * pMobIndex)
{
    ...
    STR_DIF( pMobIndex->affected_by, race_table[race].aff, temp2, AFF_FLAGS );
    fprintf( fp, "%s ", str_fwrite_flag( temp2,	AFF_FLAGS, buf ) );
	...
	STR_DIF(race_table[race].aff,pMobIndex->affected_by,temp2,AFF_FLAGS);
    if (!STR_IS_ZERO(temp2,AFF_FLAGS))
     	fprintf( fp, "F aff %s\n", str_fwrite_flag( temp2, AFF_FLAGS, buf ) );
 ...
 */
    ...
    switch (pObjIndex->item_type)
    {
        ...
        case ITEM_HEAL_FIXED:
            ...
            pObjIndex->value[1] > 0 ? pObjIndex->value[1] : 0,
            pObjIndex->value[2] > 0 ? pObjIndex->value[2] : 0,
            pObjIndex->value[3] > 0 ? pObjIndex->value[3] : 0,
            pObjIndex->value[4] > 0 ? pObjIndex->value[4] : 0
                     );
            break;
    ...
    for (iHash = 0; iHash < MAX_KEY_HASH; iHash++)
    {
             ...
             pRoomIndex = pRoomIndex->next)
        {
            if (pRoomIndex->area == pArea)
            {
                ...
                { 
                fprintf (fp, "%d ", pRoomIndex->monster_vnum[door]);               
                }
                fprintf (fp, "\n");   
