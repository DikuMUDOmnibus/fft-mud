...
void fwrite_char (CHAR_DATA * ch, FILE * fp)
{
    ...
    fprintf (fp, "Attr %d %d %d %d %d\n",
             ...
             ch->Faith,
             ch->MagAttack,
             ch->perm_stat[STAT_DEX], ch->Brave);

    fprintf (fp, "AMod %d %d %d %d %d\n",
             ch->mod_stat[STAT_PA],
             ch->mod_stat[STAT_FAITH],
             ch->mod_stat[STAT_MA],
             ch->mod_stat[STAT_DEX], ch->mod_stat[STAT_BRAVE]);
...

{
        ...
        fprintf (fp, "CJobL ");
        for (jc = 0; jc < MAX_JOB; jc++)
        fprintf (fp, "%d ", ch->job_level[jc]);
        fprintf (fp, "\n");

        fprintf (fp, "CJobT ");
        for (jc = 0; jc < MAX_JOB; jc++)
        fprintf (fp, "%d ", ch->job_top_jp[jc]);
        fprintf (fp, "\n");

        fprintf (fp, "CJobJ ");
        for (jc = 0; jc < MAX_JOB; jc++)
        fprintf (fp, "%d ", ch->job_jp[jc]);
        fprintf (fp, "\n");
}
        /* write alias */
    ...
    for (;;)
    {
        ...
        switch (UPPER (word[0]))
        {
            ...
            case 'A':
				...
				if ( ch->version < 6 )
				{
	    			KEYF( "AffectedBy", ch->affected_by, aff_convert_fread_flag(
	    									fread_flag( fp ) ), AFF_FLAGS );
	    			KEYF( "AfBy", ch->affected_by, aff_convert_fread_flag(
	    									fread_flag( fp ) ), AFF_FLAGS );
	    		}
	    		else
	    		{
	    			KEYF( "AfBy", ch->affected_by, str_fread_flag( fp, AFF_FLAGS ), AFF_FLAGS );
	    		}
            ...
            case 'C':
                ...
                if (!str_cmp (word, "CJobJ"))
                {
	        int jc;
       		 for (jc = 0; jc < MAX_JOB; jc++)
        	ch->job_jp[jc] = fread_number(fp);
                    fMatch = TRUE;            
	          	}
	          	
	            if (!str_cmp (word, "CJobT"))
                {
	        int jc;
       		 for (jc = 0; jc < MAX_JOB; jc++)
        	ch->job_top_jp[jc] = fread_number(fp);
                    fMatch = TRUE;            
	          	}

                if (!str_cmp (word, "CJobL"))
                {
       		 int jc;
	        for (jc = 0; jc < MAX_JOB; jc++)
	        ch->job_level[jc] = fread_number(fp);
                    fMatch = TRUE;            
		}
            ...
            case 'E':
                ...
                KEY ("eHPP", ch->HPP, fread_number (fp));
                KEY ("eMPP", ch->MPP, fread_number (fp));
                KEY ("eSpP", ch->SpP, fread_number (fp));
                KEY ("ePAP", ch->PAP, fread_number (fp));
                KEY ("eMAP", ch->MAP, fread_number (fp));
                KEY ("eFaith", ch->Faith, fread_number (fp));
                KEY ("eBrave", ch->Brave, fread_number (fp));
                break;

            case 'F':
                KEY ("Fdisp", ch->fdisp, fread_string (fp));
		break;
...
if (!str_cmp (word, "JobJ"))
{
 		...
 		break;
            ...
            case 'S':
                ...
                KEY ("SkJob1", ch->jobid, fread_number (fp));
                KEY ("SkJob2", ch->job2id, fread_number (fp));
                KEY ("SkReact", ch->reaction_num, fread_number (fp));
                KEY ("SkMove", ch->move_num, fread_number (fp));
                KEY ("SkMisc", ch->misc_num, fread_number (fp));
